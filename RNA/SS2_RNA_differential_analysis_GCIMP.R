# ============================================================
# GCIMP Differential Expression and Heatmap Analysis
...
# ---- Subset object ----
# Remove normal samples and those without GCIMP classification
...
# Compare GCIMP_High vs GCIMP_Low, controlling for patient effects
...
# Add gene names
diff_analysis_gcimp$gene = rownames(diff_analysis_gcimp)
...
average_expr = AverageExpression(
  ...
  features = unique(subset(diff_analysis_gcimp, abs(avg_log2FC) > 2 & p_val_adj < 0.01)$gene)
)$RNA

# Clean up column names
colnames(average_expr) = gsub("-", "_", colnames(average_expr))
colnames(average_expr) = sample_final_names_2[colnames(average_expr), ]$new_name
...
annotation_df = data.frame(
  ...
  subset(count > 10 | Sample_Final == "TKU3095")

# Filter only relevant samples
annotation_df = annotation_df[grep('MD|SM|TK', annotation_df$Sample_Final), ]
